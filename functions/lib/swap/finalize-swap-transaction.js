...
function ownerWardrobePath(ownerId, itemId, sourcePath) {
    return `users/${ownerId}/wardrobe/${itemId}/original.${extensionFromPath(sourcePath)}`;
}
async function markFinalizationFailed(transactionId, errorCode) {
...
export async function finalizeSwapTransaction(transactionId) {
    ...
    try {
        ...
        const requestedWardrobeItemId = requiredString(swap, 'requestedWardrobeItemId');
        ...
        const [listingSnapshot, requestedSnapshot, offeredSnapshot] = await Promise.all([
            listingRef.get(),
        ...
        if (requestedItem.ownerId !== listingOwnerId ||
        ...
        requestedTargetPath = ownerWardrobePath(requesterId, requestedWardrobeItemId, requestedSourcePath);
        offeredTargetPath = ownerWardrobePath(listingOwnerId, offeredWardrobeItemId, offeredSourcePath);
        ...
        await db.runTransaction(async (transaction) => {
            const [latestTransactionSnapshot, latestListingSnapshot, latestRequestedSnapshot, latestOfferedSnapshot,] = await Promise.all([
            ...
            transaction.delete(db.collection('swapOfferKeys').doc(`${listingId}_${requesterId}`));
        });
        ...
        const cleanupTargets = [requestedTargetPath, offeredTargetPath].filter((path) => typeof path === 'string');
        await Promise.allSettled(cleanupTargets.map((path) => bucket.file(path).delete({ ignoreNotFound: true })));
        await markFinalizationFailed(transactionId, 'FINALIZATION_FAILED');
        throw error;
    }
}
//# sourceMappingURL=finalize-swap-transaction.js.map