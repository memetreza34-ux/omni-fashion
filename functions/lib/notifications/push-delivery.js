...
async function claimDelivery(notificationId, device) {
...
export const onNotificationCreatedPushDelivery = onDocumentCreated({
    ...
    try {
        ...
        for (const { deliveryId } of claims) {
            batch.set(db.collection('pushDeliveries').doc(deliveryId), {
                status: 'send_failed',
    ...
    const rawTickets = isRecord(payload) && Array.isArray(payload.data) ? payload.data : [];
    const batch = db.batch();
    for (let index = 0; index < claims.length; index += 1) {
        ...
        if (ticket.status === 'ok' && ticket.id) {
            ...
            batch.set(db.collection('pushDevices').doc(claim.device.id), {
