...
import { outerwearNeedFor, rainProtectionRecommended, temperatureBandFor, } from '../weather/weather-normalization.js';
...
async function fetchJson(url) {
    ...
    try {
        ...
        throw new HttpsError('unavailable', 'Wetterdaten sind vorübergehend nicht verfügbar.');
    }
    if (!response.ok) {
    ...
    try {
        ...
        throw new HttpsError('internal', 'Wetterdaten konnten nicht verarbeitet werden.');
    }
}
async function geocodeCity(city) {
    ...
    if (!isRecord(raw) ||
        ...
        throw new HttpsError('not-found', 'Diese Stadt konnte nicht eindeutig gefunden werden.');
    }
...
function readNullableProbability(value) {
...
async function loadWeather(location) {
    ...
    const probability = readNullableProbability(raw.daily.precipitation_probability_max);
    ...
    return {
        weather: {
            ...
            outerwearNeed: outerwearNeedFor(apparentTemperatureC, precipitationMm, windSpeedKmh),
            rainProtectionRecommended: rainProtectionRecommended(precipitationMm, rainMm, probability),
...
export const getOutfitWeather = onCall({
    ...
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Du musst angemeldet sein, um Wetterdaten zu laden.');
    }
    const input = parseRequest(request.data);
    const location = await geocodeCity(input.city);
    return loadWeather(location);
});
//# sourceMappingURL=get-outfit-weather.js.map