...
function parseRequest(data) {
    ...
    if (typeof transactionId !== 'string' ||
        ...
        !DISPUTE_REASONS.includes(reason) ||
...
function readOpenTransactionStatus(value) {
    if (value === 'accepted' ||
    ...
    throw new HttpsError('failed-precondition', 'Der aktuelle Trade-Status kann nicht sicher als Streitfall gespeichert werden.');
}
export const openSwapDispute = onCall({ region: FUNCTIONS_REGION, timeoutSeconds: 30, memory: '256MiB' }, async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (disputeSnapshot.exists) {
            throw new HttpsError('already-exists', 'Für diesen Trade existiert bereits ein Klärungsfall.');
        }
        ...
        if (swap.status === 'completed' ||
            ...
            throw new HttpsError('failed-precondition', 'Dieser Trade kann im aktuellen Zustand nicht in einen Klärungsfall überführt werden.');
        }
...
//# sourceMappingURL=open-swap-dispute.js.map