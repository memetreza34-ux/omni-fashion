...
function requireRecentAuthentication(token) {
    if (!isRecord(token) || typeof token.auth_time !== 'number') {
        throw new HttpsError('failed-precondition', 'Für die Kontolöschung ist eine erneute Anmeldung erforderlich.');
    }
    const ageSeconds = Math.floor(Date.now() / 1000) - token.auth_time;
    if (ageSeconds < 0 || ageSeconds > RECENT_AUTH_SECONDS) {
        throw new HttpsError('failed-precondition', 'Die Anmeldung ist für diese sensible Aktion nicht mehr aktuell. Bitte erneut anmelden.');
    }
}
function pseudonymForUser(uid) {
    return `deleted_${createHash('sha256')
        .update(`omni-fashion-deleted:${uid}`)
        .digest('hex')
        .slice(0, 24)}`;
}
function activeStatus(value, allowed) {
...
export const deleteMyAccount = onCall({
    ...
    const [listingsSnapshot, requesterOffersSnapshot, ownerOffersSnapshot, transactionsSnapshot, locksSnapshot,] = await Promise.all([
    ...
    const blockers = [];
    if (listingsSnapshot.docs.some((document) => activeStatus(document.data().status, ['active', 'paused', 'reserved']))) {
        blockers.push('ACTIVE_SWAP_LISTING');
    }
    if ([...requesterOffersSnapshot.docs, ...ownerOffersSnapshot.docs].some((document) => document.data().status === 'sent')) {
        blockers.push('ACTIVE_SWAP_OFFER');
    }
    if (transactionsSnapshot.docs.some((document) => !activeStatus(document.data().status, ['completed', 'cancelled']))) {
    ...
    if (blockers.length > 0) {
        throw new HttpsError('failed-precondition', 'Das Konto kann erst gelöscht werden, wenn alle offenen OmniSwap-Vorgänge abgeschlossen oder beendet sind.', { blockers: [...new Set(blockers)] });
    }
    const pseudonym = pseudonymForUser(uid);
    const [wardrobeSnapshot, outfitsSnapshot, disputesSnapshot, authoredReviewsSnapshot, receivedReviewsSnapshot, outgoingBlocksSnapshot, incomingBlocksSnapshot, notificationsSnapshot, reportsByReporterSnapshot, reportsByOwnerSnapshot, reportsByTargetSnapshot, pushDevicesSnapshot, pushDeliveriesSnapshot, pushTicketsSnapshot, offerKeysSnapshot, rateLimitsSnapshot,] = await Promise.all([
    ...
    for (const document of uniqueDocuments([
        ...
        if (raw.requesterId === uid)
            update.requesterId = pseudonym;
        if (raw.listingOwnerId === uid)
            update.listingOwnerId = pseudonym;
    ...
    for (const document of transactionsSnapshot.docs) {
        ...
        if (!isRecord(raw))
            continue;
        const update = {
            participantIds: participantIds(raw.participantIds).map((id) => id === uid ? pseudonym : id),
            updatedAt: FieldValue.serverTimestamp(),
        };
        if (raw.requesterId === uid)
            update.requesterId = pseudonym;
    ...
    for (const document of uniqueDocuments([
        ...
        if (raw.reviewerId === uid)
            update.reviewerId = pseudonym;
        if (raw.revieweeId === uid)
            update.revieweeId = pseudonym;
        writer.update(document.ref, update);
        pseudonymizedHistoricalDocuments += 1;
    }
    for (const document of uniqueDocuments([
        ...
        if (raw.reporterId === uid)
            update.reporterId = pseudonym;
        if (raw.targetOwnerId === uid)
            update.targetOwnerId = pseudonym;
...
//# sourceMappingURL=delete-my-account.js.map