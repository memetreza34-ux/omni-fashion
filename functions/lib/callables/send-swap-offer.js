...
function parseRequest(data) {
    ...
    return {
        ...
        offeredWardrobeItemId: parseId(data.offeredWardrobeItemId, 'offeredWardrobeItemId'),
    };
}
function requiredString(record, field) {
    const value = record[field];
    if (typeof value !== 'string' || !value.trim()) {
        throw new HttpsError('failed-precondition', `Benötigtes Feld ${field} fehlt.`);
    }
    return value.trim();
}
...
function snapshotFromWardrobe(itemId, item) {
    return {
...
export const sendSwapOffer = onCall({
    ...
    await db.runTransaction(async (transaction) => {
        const [listingSnapshot, offeredSnapshot, lockSnapshot, keySnapshot] = await Promise.all([
        ...
        if (!offeredSnapshot.exists) {
            throw new HttpsError('not-found', 'Dein angebotenes Kleidungsstück wurde nicht gefunden.');
        }
        if (lockSnapshot.exists) {
            throw new HttpsError('failed-precondition', 'Dieses Kleidungsstück steckt bereits in einem aktiven Tauschangebot.');
        }
        if (keySnapshot.exists) {
            throw new HttpsError('already-exists', 'Du hast für dieses Listing bereits ein aktives Angebot.');
        }
        ...
        if (listingOwnerId === requesterId) {
            throw new HttpsError('failed-precondition', 'Du kannst dein eigenes Listing nicht ertauschen.');
        }
        const [requesterBlocksOwner, ownerBlocksRequester] = await Promise.all([
            transaction.get(db.collection('blocks').doc(`${requesterId}_${listingOwnerId}`)),
            transaction.get(db.collection('blocks').doc(`${listingOwnerId}_${requesterId}`)),
        ]);
        if (requesterBlocksOwner.exists || ownerBlocksRequester.exists) {
            throw new HttpsError('permission-denied', 'Zwischen diesen Konten sind keine neuen Tauschangebote möglich.');
        }
        if (listing.status !== 'active') {
            throw new HttpsError('failed-precondition', 'Dieses Listing ist nicht mehr verfügbar.');
        }
        if (offeredItem.ownerId !== requesterId) {
            throw new HttpsError('permission-denied', 'Das angebotene Kleidungsstück gehört nicht zu deinem Schrank.');
        }
        if (offeredItem.isListedForSwap === true || offeredItem.swapListingId) {
            throw new HttpsError('failed-precondition', 'Ein bereits gelistetes Kleidungsstück kann nicht gleichzeitig angeboten werden.');
        }
        ...
        transaction.set(offerRef, {
            ...
            offeredSnapshot: snapshotFromWardrobe(input.offeredWardrobeItemId, offeredItem),
            status: 'sent',
            transactionId: null,
            createdAt: now,
            updatedAt: now,
...
//# sourceMappingURL=send-swap-offer.js.map