...
function offerKeyId(listingId, requesterId) {
    return `${listingId}_${requesterId}`;
}
async function expireCompetingOffers(acceptedOfferId, listingId) {
    const db = getFirestore();
    const snapshot = await db
        .collection('swapOffers')
        .where('requestedListingId', '==', listingId)
        .get();
    const competing = snapshot.docs.filter((doc) => doc.id !== acceptedOfferId && doc.data().status === 'sent');
    ...
    for (const doc of competing.slice(0, 400)) {
        ...
        batch.delete(db.collection('swapOfferKeys').doc(offerKeyId(listingId, requesterId)));
    }
    await batch.commit();
}
export const respondSwapOffer = onCall({
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const requestedWardrobeItemId = requiredString(offer, 'requestedWardrobeItemId');
        if (listingOwnerId !== uid) {
            throw new HttpsError('permission-denied', 'Nur die Listing-Eigentümerin oder der Listing-Eigentümer kann antworten.');
        }
        if (offer.status !== 'sent') {
            throw new HttpsError('failed-precondition', 'Dieses Angebot ist nicht mehr offen.');
        }
        ...
        const [listingSnapshot, offeredItemSnapshot, lockSnapshot, keySnapshot] = await Promise.all([
        ...
        if (!listingSnapshot.exists || !offeredItemSnapshot.exists) {
            throw new HttpsError('failed-precondition', 'Mindestens ein Kleidungsstück ist nicht mehr verfügbar.');
        }
        ...
        if (!listing || !offeredItem || !lock || !key) {
            throw new HttpsError('failed-precondition', 'Das Tauschangebot besitzt keinen gültigen Reservierungs-Lock.');
        }
        if (listing.status !== 'active') {
            throw new HttpsError('failed-precondition', 'Das Listing ist nicht mehr verfügbar.');
        }
        ...
        if (listing.shippingEnabled !== true && listing.meetupEnabled !== true) {
            throw new HttpsError('failed-precondition', 'Das Listing hat keinen gültigen Tauschweg.');
        }
        if (offeredItem.ownerId !== requesterId) {
            throw new HttpsError('failed-precondition', 'Das angebotene Kleidungsstück gehört nicht mehr zum anfragenden Konto.');
        }
        if (offeredItem.isListedForSwap === true || offeredItem.swapListingId) {
            throw new HttpsError('failed-precondition', 'Das angebotene Kleidungsstück wurde zwischenzeitlich anderweitig gelistet.');
        }
        if (lock.offerId !== input.offerId || key.offerId !== input.offerId) {
            throw new HttpsError('failed-precondition', 'Der Reservierungs-Lock gehört nicht mehr zu diesem Angebot.');
        }
        const now = FieldValue.serverTimestamp();
    ...
    return {
        offerId: input.offerId,
        status: input.decision === 'accept'
            ? 'accepted'
            : 'declined',
        transactionId,
    };
});
//# sourceMappingURL=respond-swap-offer.js.map