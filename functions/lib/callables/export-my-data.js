...
export const exportMyData = onCall({
    ...
    const [profileSnapshot, styleProfileSnapshot, wardrobeSnapshot, outfitsSnapshot, listingsSnapshot, requesterOffersSnapshot, ownerOffersSnapshot, transactionsSnapshot, disputesSnapshot, authoredReviewsSnapshot, receivedReviewsSnapshot, blocksSnapshot, notificationsSnapshot, notificationPreferencesSnapshot, reportsSnapshot, pushDevicesSnapshot,] = await Promise.all([
    ...
    const profileRaw = profileSnapshot.data();
    const styleProfileRaw = styleProfileSnapshot.data();
    const preferenceRaw = notificationPreferencesSnapshot.data();
    ...
    return {
        ...
        userId: uid,
        profile: profileSnapshot.exists && isRecord(profileRaw)
            ? documentData(profileSnapshot.id, profileRaw)
            : null,
        styleProfile: styleProfileSnapshot.exists && isRecord(styleProfileRaw)
        ...
        notificationPreferences: notificationPreferencesSnapshot.exists && isRecord(preferenceRaw)
...
//# sourceMappingURL=export-my-data.js.map