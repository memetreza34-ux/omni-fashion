...
function nextStatus(currentStatus, action) {
    ...
    throw new HttpsError('failed-precondition', 'Diese Listing-Aktion ist im aktuellen Status nicht erlaubt.');
}
async function expireOpenOffers(listingId) {
    ...
    for (const doc of sentOffers.slice(0, 400)) {
        ...
        batch.delete(db.collection('swapOfferKeys').doc(`${listingId}_${requesterId}`));
    }
    await batch.commit();
}
export const setSwapListingStatus = onCall({
    ...
    if (targetStatus === 'removed') {
        try {
            ...
            throw new HttpsError('internal', 'Öffentliches Listing-Bild konnte nicht sicher entfernt werden.');
        }
    }
    try {
        await db.runTransaction(async (transaction) => {
            ...
            if (!listingSnapshot.exists || !itemSnapshot.exists) {
                throw new HttpsError('failed-precondition', 'Listing oder Kleidungsstück ist nicht mehr verfügbar.');
            }
            ...
            if (item.isListedForSwap !== true ||
                item.swapListingId !== input.listingId) {
                throw new HttpsError('failed-precondition', 'Wardrobe-Verknüpfung zum Listing ist inkonsistent.');
            }
            const now = FieldValue.serverTimestamp();
...
//# sourceMappingURL=set-swap-listing-status.js.map