...
function readTrimmedString(value, fieldName, maxLength, allowEmpty = false) {
...
function parseRequest(data) {
    ...
    const wardrobeItemId = readTrimmedString(data.wardrobeItemId, 'wardrobeItemId', 160);
    if (wardrobeItemId.includes('/')) {
        throw new HttpsError('invalid-argument', 'wardrobeItemId ist ungültig.');
    }
    const description = readTrimmedString(data.description, 'Beschreibung', MAX_DESCRIPTION_LENGTH, true);
    ...
    if (typeof shippingEnabled !== 'boolean' ||
        ...
        throw new HttpsError('invalid-argument', 'Mindestens Versand oder Übergabe muss aktiviert sein.');
    }
...
function requiredString(record, fieldName) {
    ...
    if (typeof value !== 'string' || !value.trim()) {
        throw new HttpsError('failed-precondition', `Kleidungsstück hat kein gültiges Feld ${fieldName}.`);
    }
    return value.trim();
}
function nullableString(record, fieldName) {
    const value = record[fieldName];
    return typeof value === 'string' && value.trim() ? value.trim() : null;
}
function publicFilePath(listingId, sourcePath) {
...
export const createSwapListing = onCall({
    ...
    if (!initialData || initialData.ownerId !== uid) {
        throw new HttpsError('permission-denied', 'Dieses Kleidungsstück gehört nicht zu deinem Schrank.');
    }
    if (initialData.isListedForSwap === true || initialData.swapListingId) {
        throw new HttpsError('already-exists', 'Dieses Kleidungsstück ist bereits für OmniSwap gelistet.');
    }
    const sourceImagePath = requiredString(initialData, 'imagePath');
    if (!sourceImagePath.startsWith(`users/${uid}/wardrobe/`)) {
        throw new HttpsError('failed-precondition', 'Das private Kleidungsbild hat einen ungültigen Pfad.');
    }
    ...
    if (!sourceExists) {
        throw new HttpsError('failed-precondition', 'Das Kleidungsbild ist nicht mehr verfügbar.');
    }
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (!currentSnapshot.exists) {
                throw new HttpsError('failed-precondition', 'Das Kleidungsstück wurde während der Erstellung entfernt.');
            }
            ...
            if (item.isListedForSwap === true || item.swapListingId) {
                throw new HttpsError('already-exists', 'Dieses Kleidungsstück ist bereits gelistet.');
            }
            if (item.imagePath !== sourceImagePath) {
                throw new HttpsError('aborted', 'Das Kleidungsbild wurde während der Erstellung geändert.');
            }
        ...
        throw new HttpsError('internal', 'OmniSwap-Listing konnte nicht erstellt werden.');
    }
    return {
...
//# sourceMappingURL=create-swap-listing.js.map