...
function readNullableString(
  ...
  if (normalized.length > maxLength) {
    throw new GarmentAnalysisError(
      'INVALID_PROVIDER_OUTPUT',
      `Invalid nullable string length: ${key}`,
    );
  }
...
function readStringArray(
  ...
  return value.map((entry) => {
    ...
    if (!normalized || normalized.length > maxItemLength) {
      throw new GarmentAnalysisError(
        'INVALID_PROVIDER_OUTPUT',
        `Invalid array entry length: ${key}`,
      );
    }
    return normalized;
  });
...
function parseFieldConfidence(value: unknown): GarmentFieldConfidence {
  if (!isRecord(value)) {
    throw new GarmentAnalysisError(
      'INVALID_PROVIDER_OUTPUT',
      'Invalid fieldConfidence object.',
    );
  }
...
export function parseGarmentProviderResult(
  ...
  if (!readBoolean(value, 'isGarment')) {
    throw new GarmentAnalysisError(
      'IMAGE_NOT_GARMENT',
      'No analyzable garment detected.',
    );
  }
  ...
  return {
    ...
    subcategory:
      fieldConfidence.subcategory >= 0.45
