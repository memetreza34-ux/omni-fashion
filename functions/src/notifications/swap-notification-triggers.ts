...
export const onSwapOfferCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (!listingOwnerId || !listingId) return;
...
export const onSwapTransactionUpdated = onDocumentUpdated(
  ...
  async (event) => {
    ...
    const eventForNewActor = async (
      beforeValues: unknown,
      afterValues: unknown,
      kind: 'mode' | 'shipped' | 'received',
    ) => {
    ...
    await eventForNewActor(
      before.modeConfirmedByIds,
      after.modeConfirmedByIds,
      'mode',
    );
...
export const onSwapDisputeCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (participants.length !== 2 || !openedById) return;
