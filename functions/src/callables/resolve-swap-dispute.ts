...
function resumableStatus(value: unknown): ResumableStatus {
  if (
    value === 'accepted' ||
  ...
  throw new HttpsError(
    'failed-precondition',
    'Der gespeicherte Trade-Zustand kann nicht sicher wiederhergestellt werden.',
  );
}
  ...
  async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (!disputeSnapshot.exists || !swapSnapshot.exists) {
        throw new HttpsError(
          'not-found',
          'Streitfall oder Trade wurde nicht gefunden.',
        );
      }
      ...
      if (dispute.status !== 'open' || swap.status !== 'disputed') {
        throw new HttpsError(
          'failed-precondition',
          'Dieser Streitfall ist nicht mehr offen.',
        );
      }
      if (
        ...
        throw new HttpsError(
          'failed-precondition',
          'Der Trade befindet sich bereits in einer nicht sicher rücksetzbaren Finalisierung.',
        );
      }

      const now = FieldValue.serverTimestamp();
      let restoredStatus: ResumableStatus | null = null;
