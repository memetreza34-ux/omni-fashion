...
import {
  ...
  SwapProgressError,
} from '../swap/transaction-state.js';
import type {
...
interface AdvanceSwapTransactionInput {
...
function parseAction(value: unknown): SwapProgressAction {
...
function parseRequest(data: unknown): AdvanceSwapTransactionInput {
  if (!isRecord(data)) {
...
function parseParticipants(value: unknown): [string, string] {
  if (
    !Array.isArray(value) ||
    value.length !== 2 ||
...
function parseMode(value: unknown): SwapFulfilmentMode | null {
  if (value === null) {
    return null;
  }
...
function parseStatus(value: unknown): SwapTransactionStatus {
  if (
    value === 'accepted' ||
...
function parseFinalizationState(value: unknown): SwapFinalizationState {
  if (
    value === 'pending' ||
    value === 'ready' ||
    value === 'processing' ||
...
function parseProgress(data: Record<string, unknown>): SwapProgressState {
  if (data.schemaVersion !== TRANSACTION_SCHEMA_VERSION) {
    throw new HttpsError(
      'failed-precondition',
      'Diese Trade-Version muss vor der Fortsetzung migriert werden.',
    );
  }
...
function validateModeAllowed(
  listing: Record<string, unknown>,
  mode: SwapFulfilmentMode,
): void {
  if (mode === 'shipping' && listing.shippingEnabled !== true) {
    throw new HttpsError(
      'failed-precondition',
      'Versand ist für dieses Listing nicht freigegeben.',
    );
  }
  if (mode === 'meetup' && listing.meetupEnabled !== true) {
    throw new HttpsError(
      'failed-precondition',
      'Persönliche Übergabe ist für dieses Listing nicht freigegeben.',
    );
  }
}
...
export const advanceSwapTransaction = onCall(
  ...
  async (request) => {
    ...
    let updatedState: SwapProgressState;
      ...
      updatedState = await db.runTransaction(async (transaction) => {
        ...
        if (input.action.type === 'confirm_mode') {
          ...
          const listingSnapshot = await transaction.get(
            db.collection('swapListings').doc(listingId),
          );
          const listing = listingSnapshot.data();
          if (!listing) {
            throw new HttpsError(
              'failed-precondition',
              'Das zugehörige Listing fehlt.',
            );
          }
    ...
    if (updatedState.finalizationState === 'ready') {
      try {
        ...
        throw new HttpsError(
          'internal',
          'Der Tausch wurde bestätigt, aber die Eigentumsübertragung konnte nicht sicher abgeschlossen werden. Ein erneuter Abschlussversuch ist möglich.',
        );
      }
    }
