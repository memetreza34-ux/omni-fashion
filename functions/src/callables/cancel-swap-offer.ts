...
function parseOfferId(data: unknown): string {
  if (!isRecord(data)) {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (requesterId !== uid) {
        throw new HttpsError(
          'permission-denied',
          'Nur die anfragende Person kann dieses Angebot zurückziehen.',
        );
      }
      if (offer.status !== 'sent') {
        throw new HttpsError(
          'failed-precondition',
          'Nur offene Angebote können zurückgezogen werden.',
        );
      }
      ...
      if (
        ...
        throw new HttpsError(
          'failed-precondition',
          'Der Reservierungs-Lock dieses Angebots ist inkonsistent.',
        );
      }
