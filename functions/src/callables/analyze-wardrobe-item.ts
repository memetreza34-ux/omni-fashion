...
import {
  ...
  GARMENT_ANALYSIS_PROMPT_VERSION,
  GARMENT_ANALYSIS_SCHEMA_VERSION,
...
interface AnalyzeWardrobeItemInput {
...
interface CachedPreparation {
  kind: 'cached';
  response: GarmentAnalysisResponse;
}

type Preparation = RunPreparation | CachedPreparation;
...
function parseRequest(data: unknown): AnalyzeWardrobeItemInput {
  if (!isRecord(data)) {
  ...
  if (schemaVersion !== GARMENT_ANALYSIS_SCHEMA_VERSION) {
    throw new HttpsError(
      'failed-precondition',
      'Diese App-Version verwendet ein nicht unterstütztes Analyseschema.',
    );
  }
...
function validateImagePath(
  ...
  if (typeof imagePath !== 'string') {
    throw new HttpsError(
      'failed-precondition',
      'Für dieses Kleidungsstück ist kein analysierbares Bild gespeichert.',
    );
  }
  ...
  if (!imagePath.startsWith(expectedPrefix)) {
    throw new HttpsError(
      'permission-denied',
      'Der Bildpfad gehört nicht zu diesem Kleidungsstück.',
    );
  }
...
function failureCodeFrom(error: unknown): GarmentAnalysisFailureCode {
...
function clientErrorFor(code: GarmentAnalysisFailureCode): HttpsError {
  switch (code) {
    case 'IMAGE_NOT_FOUND':
      return new HttpsError(
        'not-found',
        'Das Kleidungsbild wurde nicht gefunden.',
      );
    case 'IMAGE_INVALID':
      return new HttpsError(
        'failed-precondition',
        'Das gespeicherte Bild kann nicht analysiert werden.',
      );
    case 'IMAGE_NOT_GARMENT':
      return new HttpsError(
        'failed-precondition',
        'Auf dem Bild wurde kein eindeutiges Kleidungsstück erkannt.',
      );
    ...
    case 'PROVIDER_UNAVAILABLE':
      return new HttpsError(
        'unavailable',
        'Die Kleidungsanalyse ist vorübergehend nicht verfügbar.',
      );
    case 'INVALID_PROVIDER_OUTPUT':
    case 'INTERNAL_ERROR':
      return new HttpsError(
        'internal',
        'Die Kleidungsanalyse konnte nicht abgeschlossen werden.',
      );
  }
}
...
async function loadImage(
  ...
  try {
    ...
    throw new GarmentAnalysisError(
      'IMAGE_NOT_FOUND',
      'Wardrobe image does not exist in Storage.',
    );
  }
  ...
  if (
    ...
    throw new GarmentAnalysisError(
      'IMAGE_INVALID',
      'Wardrobe image metadata is invalid for analysis.',
    );
  }
  ...
  try {
    ...
    throw new GarmentAnalysisError(
      'IMAGE_NOT_FOUND',
      'Wardrobe image could not be downloaded.',
    );
  }
  ...
  if (buffer.length <= 0 || buffer.length >= MAX_IMAGE_BYTES) {
    throw new GarmentAnalysisError(
      'IMAGE_INVALID',
      'Wardrobe image bytes are invalid for analysis.',
    );
  }
...
async function markAnalysisFailed(
  wardrobeItemId: string,
  failureCode: GarmentAnalysisFailureCode,
): Promise<void> {
  ensureAdminInitialized();
...
async function prepareAnalysis(
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (!isRecord(raw)) {
      throw new HttpsError(
        'failed-precondition',
        'Kleidungsstück hat ungültige Daten.',
      );
    }
    ...
    if (raw.ownerId !== userId) {
      throw new HttpsError(
        'permission-denied',
        'Dieses Kleidungsstück gehört nicht zu deinem Konto.',
      );
    }
    ...
    if (raw.aiStatus === 'pending' && raw.updatedAt instanceof Timestamp) {
      ...
      if (pendingAgeMs >= 0 && pendingAgeMs < PENDING_STALE_AFTER_MS) {
        throw new HttpsError(
          'failed-precondition',
          'Für dieses Kleidungsstück läuft bereits eine Analyse.',
        );
      }
    }
...
export const analyzeWardrobeItem = onCall(
  ...
  async (request): Promise<GarmentAnalysisResponse> => {
    if (!request.auth) {
      throw new HttpsError(
        'unauthenticated',
        'Du musst angemeldet sein, um ein Kleidungsstück zu analysieren.',
      );
    }
