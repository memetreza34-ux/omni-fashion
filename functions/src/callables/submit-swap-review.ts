...
function participants(value: unknown): [string, string] {
  if (
    !Array.isArray(value) ||
    value.length !== 2 ||
...
export const submitSwapReview = onCall(
  ...
  async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (reviewSnapshot.exists) {
        throw new HttpsError(
          'already-exists',
          'Du hast diesen Trade bereits bewertet.',
        );
      }
      ...
      if (
        ...
        throw new HttpsError(
          'failed-precondition',
          'Bewertungen sind erst nach vollständig abgeschlossenem Tausch möglich.',
        );
      }
    ...
    return { reviewId, created: true as const };
  },
);
