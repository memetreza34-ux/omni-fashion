...
test('M3 Navigation: Web Tab Navigation (app-tabs.web.tsx) contains all 5 routes and UI polish elements', () => {
  ...
  assert.ok(content.includes('name="index"') && content.includes('href="/"'), 'Must include index route');
  assert.ok(content.includes('name="stylist"') && content.includes('href="/stylist"'), 'Must include stylist route');
  assert.ok(content.includes('name="shop"') && content.includes('href="/shop"'), 'Must include shop route');
  assert.ok(content.includes('name="swap"') && content.includes('href="/swap"'), 'Must include swap route');
  assert.ok(content.includes('name="profile"') && content.includes('href="/profile"'), 'Must include profile route');
  ...
  assert.ok(content.includes('label="OmniSwap"'), 'Must include OmniSwap label');
  ...
  assert.ok(content.includes('icon={ShoppingBag}'), 'Must include ShoppingBag icon');
  ...
  assert.ok(content.includes('backdropFilter') || content.includes('blur('), 'Must include glassmorphism backdrop filter');
  assert.ok(content.includes('Omni-Fashion'), 'Must include brand title Omni-Fashion');
  assert.ok(content.includes('Circular Hub'), 'Must include eco badge Circular Hub');
  assert.ok(content.includes('activeIndicator'), 'Must include active tab indicator styling');
});
...
test('M3 Navigation: Native Tab Navigation (app-tabs.tsx) contains swap trigger', () => {
  ...
  assert.ok(content.includes('name="swap"'), 'Must include NativeTabs.Trigger for swap');
  assert.ok(content.includes('OmniSwap'), 'Must include OmniSwap label for native tabs');
});
