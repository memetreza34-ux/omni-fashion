...
const RECOMMENDATIONS: InvestmentPiece[] = [
  {
    ...
    matchScore: 97,
    reason:
      'Schließt die Übergangs-Lücke. Passt zu all deinen T-Shirts und verleiht deinen schlichten Hosen sofort einen kantigen High-Fashion Look.',
    image:
  ...
  },
  {
    ...
    matchScore: 93,
    reason:
      'Deine Hosen sind aktuell sehr denim-lastig. Diese Chino öffnet den Weg für sommerliche Smart-Casual & Office-Kombinationen.',
    image:
  ...
  },
  {
    ...
    matchScore: 89,
    reason:
      'Ersetzt Sneaker bei schickeren Anlässen (Date Night, Business), ohne an Bequemlichkeit oder Style-Faktor einzubüßen.',
    image:
...
export function SmartInvestmentAdvisor() {
  const [selectedPiece, setSelectedPiece] = useState<InvestmentPiece>(
    RECOMMENDATIONS[0],
  );
  ...
  return (
    <View className="bg-zinc-900 rounded-[32px] p-6 border border-zinc-800 shadow-2xl mb-8">
      ...
      <Text className="text-zinc-300 text-sm leading-relaxed mb-5">
        Statt blind zu shoppen: Diese Teile maximieren deine
        Outfit-Möglichkeiten mit deinen bestehenden Sachen.
      </Text>
        ...
        {RECOMMENDATIONS.map((piece) => {
          ...
          return (
            ...
            >
              ...
              <Text className="text-blue-400 text-[11px] font-semibold mt-0.5">
                +{piece.unlockOutfits} neue Looks
              </Text>
            </TouchableOpacity>
          );
        })}
      ...
      <View className="bg-zinc-950/90 rounded-2xl p-4 border border-zinc-800">
        ...
        <View className="flex-row gap-2">
          {selectedPiece.swapAvailable ? (
            ...
            >
              <Text className="text-zinc-300 font-bold text-xs mr-1">
                🔔 Tausch-Radar stellen
              </Text>
            </TouchableOpacity>
          )}
